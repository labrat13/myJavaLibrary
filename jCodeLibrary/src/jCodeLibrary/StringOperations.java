...
public class StringOperations
{
    ...
    public static String getStringPart(String text, String from, String to)
    {
        ...
        if (startPos == -1) return null;// Error: Start pattern not founded in
                                        // file.
        int endPos = text.indexOf(to, startPos + startLen);
        if (endPos == -1) return null;// Error: End pattern not founded in file.
        // substring
    ...
    public static String[] RemoveEmptyItems(String[] sar)
    {
        ...
        for (String s : sar)
            if (!StringIsNullOrEmpty(s)) count++;
        // create result array
